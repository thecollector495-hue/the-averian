  ...
  return (
    <header className="fixed bottom-0 left-0 right-0 h-16 bg-background/80 backdrop-blur-sm border-t border-border z-50">
      <nav className="flex justify-around w-full h-full max-w-md mx-auto">
        {navItems.map((item) => {
          const isActive = (pathname === '/' && item.href === '/') || (pathname === '/birds' && item.href === '/birds') || (item.href !== '/' && item.href !== '/birds' && pathname.startsWith(item.href));
          if(item.label === 'Birds' && pathname !== '/') {
            const isActive = false;
          }
          if(item.label === 'Assistant' && pathname !== '/birds') {
            const isActive = false;
          }
          ...
          if (item.href !== '/') {
           ...
           if (pathname === '/birds' && item.href !== '/birds') {
             finalIsActive = false;
           }
            if (pathname !== '/birds' && item.href === '/birds') {
                finalIsActive = false;
          ...
          return (
            ...
            >
              <item.icon className={cn(
                  ...
                  finalIsActive ? "scale-110" : "group-hover:scale-105"
                )} />
